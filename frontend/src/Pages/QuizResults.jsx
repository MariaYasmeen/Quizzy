...
const QuizResults = () => {
  ...
  const quizId = "67359d5034c78da7944eab13";
  ...
  const totalCorrectAnswers = results.answers.filter(
    (answer) => answer.isCorrect
  ).length;
  const percentage = (
    (totalCorrectAnswers / results.answers.length) *
    100
  ).toFixed(2);
      ...
      <Row className="mt-4">
        <Col md={12}>
          <Card className="p-4 shadow-sm">
            <Card.Body>
              <h5>
                <strong>Quiz ID:</strong> {results.quiz.title}
              </h5>
              <h5>
                <strong>Student ID:</strong> {results.student.name}
              </h5>
              <h5>
                <strong>Total Questions:</strong> {results.answers.length}
              </h5>
              <h5>
